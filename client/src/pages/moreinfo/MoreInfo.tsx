...
function MoreInfo() {
    ...
    const [val, setVal] = useState<Group>()
    ...
    useEffect(() => {
        (async () => {
            const id = q.get("q")
            const { data } = await callAuthApi<Group[]>('get', `/group/viewgroup?q=${id}`)
            if (!data.success) return console.error(data.message)
            const group = data.data[0]
            setHasRequested(group?.requests?.includes(user?._id ?? ''))
            setVal(group)
    ...
    const incomment = useRef<HTMLInputElement>(null)
    ...
    async function commentPost() {
        if (!user || !val) return navigate('/login')


        const comment = incomment.current?.value
        const targetGroup = val._id

        if (!comment) return toast.error("please enter the comment", {
            style: {
                borderRadius: '10px',
                background: 'var(--toast-bg)',
                color: 'var(--toast-color)',
        ...
        if (res.status == 201) {
            ...
            if (incomment.current) incomment.current.value = ''
        }
        else {
