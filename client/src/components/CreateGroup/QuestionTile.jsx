...
function QuestionTile({ edit = 0 }) {
    ...
    async function editgroup() {
        ...
        try {
            const res = await api.post(`/group/editgroup?q=${edit}`, body)
            if (res.status == 200) {
                return navigate(`/moreinfo?q=${edit}`)
            }
        } catch (error) {
            if (error.status == 403) {
                return toast.error("Only owner of group can edit", {
                    style: {
                        borderRadius: '10px',
                        background: 'var(--toast-bg)',
                        color: 'var(--toast-color)',
