...
export const authMiddleware: RequestHandler = async (req, res, next) => {
    ...
    var decoded
    try {
        decoded = await jose.jwtVerify<{ email: string }>(accessToken, generateUni8Array(env.ACCESS_TOKEN_SECRET))
    } catch (error) {
        if (error instanceof jose.errors.JOSEError) {
            ...
            throw new CustomError(500, "INVALID_TOKEN", error.message)
        }
        throw error
    }
    ...
    const { fullName, _id, username, memberGroup: groups, avatar } = user
    req.user = { fullName, _id, email, username, groups, avatar }
